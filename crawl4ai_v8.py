...
def get_urls():
    ...
    log(f"Loaded {len(urls)} URLs, {len(done)} already done")
...
def upload(d):
    sku = d.get("sku", d["url"].split("/")[-1][:50])
    slug = sku
    ...
    try:
        # Check by SKU
        cu = f"{URL}/rest/v1/products?sku=eq.{sku}&select=id"
        ...
        log(f"Upload error {e.code}")
    except: pass
    return False

async def main():
    log("="*50)
    log("Crawl4AI v8 STARTING...")
    urls = get_urls()
    if not urls:
